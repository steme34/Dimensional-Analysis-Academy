(() => {
  ...
  const XP_STORAGE_KEY = "chemistrySkillsAcademyPassport";
  const XP_PENDING_KEY = "chemistrySkillsAcademyPendingXp";
  const XP_LEVELS = Object.freeze([
    { min: 0, name: "Academy Student" },
    { min: 250, name: "Lab Assistant" },
    { min: 600, name: "Lab Technician" },
    { min: 1200, name: "Researcher" },
    { min: 2200, name: "Senior Chemist" },
    { min: 3500, name: "Master Chemist" }
  ]);
  const ACHIEVEMENTS = Object.freeze([
    { id: "first-step", icon: "🧪", name: "First Reaction", description: "Complete any Academy activity.", test: p => p.totalCompletions >= 1 },
    { id: "academy-graduate", icon: "🎓", name: "Academy Graduate", description: "Complete all four activities once.", test: p => p.academyCycles >= 1 },
    { id: "metric-master", icon: "🎈", name: "Metric Master", description: "Complete Metric Balloon five times.", test: p => p.activityCounts["metric-balloon"] >= 5 },
    { id: "factory-foreman", icon: "🏭", name: "Factory Foreman", description: "Complete Sig Fig Factory five times.", test: p => p.activityCounts["sig-fig-factory"] >= 5 },
    { id: "notation-pro", icon: "🏎️", name: "Notation Pro", description: "Complete the Scientific Notation campaign five times.", test: p => p.activityCounts["scientific-notation"] >= 5 },
    { id: "mission-commander", icon: "🚀", name: "Mission Commander", description: "Complete the Rocket Fuel campaign five times.", test: p => p.activityCounts["rocket-fuel"] >= 5 },
    { id: "replay-scholar", icon: "🔁", name: "Replay Scholar", description: "Record twenty activity completions.", test: p => p.totalCompletions >= 20 },
    { id: "master-chemist", icon: "🏆", name: "Master Chemist", description: "Reach 3,500 XP.", test: p => p.xp >= 3500 }
  ]);
  ...
  function readPassport() {
    const fallback = defaultPassport();
    try {
      const parsed = JSON.parse(localStorage.getItem(XP_STORAGE_KEY) || "{}");
      fallback.name = typeof parsed.name === "string" ? parsed.name : "";
      fallback.xp = Number.isFinite(parsed.xp) ? Math.max(0, Math.floor(parsed.xp)) : 0;
      fallback.totalCompletions = Number.isFinite(parsed.totalCompletions) ? Math.max(0, Math.floor(parsed.totalCompletions)) : 0;
      fallback.academyCycles = Number.isFinite(parsed.academyCycles) ? Math.max(0, Math.floor(parsed.academyCycles)) : 0;
      fallback.activityCounts = parsed.activityCounts && typeof parsed.activityCounts === "object" ? parsed.activityCounts : {};
      fallback.cycleProgress = parsed.cycleProgress && typeof parsed.cycleProgress === "object" ? parsed.cycleProgress : {};
      fallback.unlockedAchievements = Array.isArray(parsed.unlockedAchievements) ? parsed.unlockedAchievements : [];
    } catch (error) { /* Use the empty passport. */ }
    return fallback;
  }

  function writePassport(passport) {
    try { localStorage.setItem(XP_STORAGE_KEY, JSON.stringify(passport)); }
    catch (error) { /* Replay progression remains optional if storage is unavailable. */ }
    return passport;
  }

  function levelForXp(xp) {
    let index = 0;
    XP_LEVELS.forEach((level, levelIndex) => { if (xp >= level.min) index = levelIndex; });
    const current = XP_LEVELS[index];
    const next = XP_LEVELS[index + 1] || null;
    return { index, number: index + 1, current, next };
  }

  function updateAchievements(passport) {
    const unlocked = new Set(passport.unlockedAchievements || []);
    const newlyUnlocked = [];
    ACHIEVEMENTS.forEach(achievement => {
      if (achievement.test(passport) && !unlocked.has(achievement.id)) {
        unlocked.add(achievement.id);
        newlyUnlocked.push(achievement.name);
      }
    });
    passport.unlockedAchievements = [...unlocked];
    return newlyUnlocked;
  }

  function queueXpMessage(message) {
    try { sessionStorage.setItem(XP_PENDING_KEY, message); }
    catch (error) { /* No toast when session storage is unavailable. */ }
  }

  function awardActivityXp(activityId) {
    const passport = readPassport();
    const previousCount = Number(passport.activityCounts[activityId]) || 0;
    const activityXp = previousCount === 0 ? 100 : 25;
    passport.activityCounts[activityId] = previousCount + 1;
    passport.totalCompletions += 1;
    passport.xp += activityXp;
    passport.cycleProgress[activityId] = true;

    let cycleBonus = 0;
    if (ACTIVITY_IDS.every(id => passport.cycleProgress[id] === true)) {
      cycleBonus = 250;
      passport.xp += cycleBonus;
      passport.academyCycles += 1;
      passport.cycleProgress = {};
    }

    const newAchievements = updateAchievements(passport);
    writePassport(passport);
    const parts = [`+${activityXp} XP`];
    if (cycleBonus) parts.push(`+${cycleBonus} Academy Cycle Bonus`);
    if (newAchievements.length) parts.push(`Achievement: ${newAchievements.join(", ")}`);
    queueXpMessage(parts.join(" • "));
    return passport;
  }

  function renderPassport() {
    const passport = readPassport();
    updateAchievements(passport);
    writePassport(passport);
    const level = levelForXp(passport.xp);
    const levelStart = level.current.min;
    const levelEnd = level.next ? level.next.min : levelStart;
    const progress = level.next ? Math.round(((passport.xp - levelStart) / (levelEnd - levelStart)) * 100) : 100;

    const setText = (id, value) => { const node = document.getElementById(id); if (node) node.textContent = value; };
    setText("passport-level-badge", `Level ${level.number}`);
    setText("passport-level-name", passport.name ? `${passport.name} — ${level.current.name}` : level.current.name);
    setText("passport-xp-text", level.next ? `${passport.xp - levelStart} / ${levelEnd - levelStart} XP to next level` : `${passport.xp} XP — Maximum Level`);
    setText("passport-total-xp", passport.xp);
    setText("passport-completions", passport.totalCompletions);
    setText("passport-cycles", passport.academyCycles);
    setText("passport-badges-count", passport.unlockedAchievements.length);
    const fill = document.getElementById("passport-xp-fill");
    if (fill) fill.style.width = `${Math.max(0, Math.min(100, progress))}%`;
    const nameInput = document.getElementById("passport-name");
    if (nameInput && document.activeElement !== nameInput) nameInput.value = passport.name;

    const achievementGrid = document.getElementById("passport-achievements");
    if (achievementGrid) {
      achievementGrid.innerHTML = "";
      const unlocked = new Set(passport.unlockedAchievements);
      ACHIEVEMENTS.forEach(achievement => {
        const item = document.createElement("div");
        item.className = `passport-achievement${unlocked.has(achievement.id) ? " unlocked" : ""}`;
        item.innerHTML = `<span class="passport-achievement__icon" aria-hidden="true">${unlocked.has(achievement.id) ? achievement.icon : "🔒"}</span><span><strong>${achievement.name}</strong><span>${achievement.description}</span></span>`;
    ...
    if (toast) {
      let message = "";
      try { message = sessionStorage.getItem(XP_PENDING_KEY) || ""; sessionStorage.removeItem(XP_PENDING_KEY); } catch (error) {}
      if (message) {
  ...
  function initializePassport() {
    const input = document.getElementById("passport-name");
    const save = document.getElementById("passport-save-name");
    if (input && save) {
      const persistName = () => {
        const passport = readPassport();
        passport.name = input.value.trim().replace(/\s+/g, " ");
        writePassport(passport);
        renderPassport();
        if (passport.name) playSound("complete");
      };
      save.addEventListener("click", persistName);
      input.addEventListener("keydown", event => { if (event.key === "Enter") persistName(); });
    }
    renderPassport();
  }
